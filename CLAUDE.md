...
- Notification icons mapped in `getNotificationIcon()` for all 5 types: STATUS_CHANGE (arrow.triangle.2.circlepath), NEW_COMMENT (bubble.left.and.bubble.right), SURGERY_SCHEDULED (calendar.badge.clock), CHECKLIST_EXPIRY (exclamationmark.triangle), SURGERY_REMINDER (bell.badge)
