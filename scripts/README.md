...
### 7. Создать тестовые уведомления (требуется поддержка бэкенда)
```bash
node scripts/seed-patient-notifications.js <JWT_TOKEN> [PATIENT_ID]
```

Создает 10 тестовых уведомлений для пациента, демонстрирующих полный жизненный цикл:
- Изменения статуса (NEW → PREPARATION → REVIEW_NEEDED → APPROVED)
- Назначение врача и хирурга
- Установка диагноза и типа операции
- Назначение даты операции
- Комментарии и обновления чек-листа

**Примечание:** Требует реализации `POST /api/v1/notifications` на бэкенде. Если endpoint не существует, скрипт покажет, какие уведомления должны быть созданы, и выведет ссылку на документацию для бэкенд-разработчиков.

**Пример:**
```bash
# Для пациента с ID 2 (Айаал Степанов, код доступа: e5f6g7h8)
node scripts/seed-patient-notifications.js <JWT_TOKEN> 2
```
